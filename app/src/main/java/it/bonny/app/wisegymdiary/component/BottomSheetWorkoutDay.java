...
public class BottomSheetWorkoutDay extends BottomSheetDialogFragment implements RecyclerViewClickBottomSheetInterface {
    ...
    private void myTask() {
        ...
        service.execute(() -> {
            List<WorkoutDay> workoutDays = AppDatabase.getInstance(context).workoutDayDAO().getAllRoutineByIdWorkPlanNoLiveData(idWorkoutPlan);
            if(getActivity() != null) {
                getActivity().runOnUiThread(() -> {
                    ListWorkoutDayBottomSheetAdapter listWorkoutDayBottomSheetAdapter = new ListWorkoutDayBottomSheetAdapter(idElementSelected, workoutDays, BottomSheetWorkoutDay.this);
